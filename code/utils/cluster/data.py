...
# Import our own tif folder class
from src.datasets.riverbank_failure_folder import RiverbankFailureFolder
...
def create_basic_clustering_dataloaders(config):
    ...
    # Using our own tif folder class
    dataset_head_B = RiverbankFailureFolder(root=train_data_path, transform=tf1)
    ...
    dataset_head_A = RiverbankFailureFolder(root=train_data_path, transform=tf1)
    datasets_tf_head_A = [RiverbankFailureFolder(root=train_data_path, transform=tf2)
                          for _ in range(config.num_dataloaders)]
